...
# Provider configuration (MCP_CLIENT_* variables, see .env.example) is supplied at
# container-run time via the environment, never baked into the image. A container run
# has no browser and no persistent home directory by default, so pass
# MCP_CLIENT_TOKEN_STORAGE_PATH pointing at a mounted volume, or leave token storage
# unset to fall back to in-memory-only (re-authenticate every run).
