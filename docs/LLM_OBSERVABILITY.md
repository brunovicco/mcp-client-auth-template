...
`src/mcp_client_auth_template/adapters/observability.py`. Install it with
...
Langfuse through `src/mcp_client_auth_template/adapters/tracing.py`. Structured application logging itself
is always configured through `src/mcp_client_auth_template/entrypoints/logging.py` and is not part of this
