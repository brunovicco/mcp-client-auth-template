...
> Support floor raised to MCP SDK 2.2.0 (`mcp>=2.2,<3`) for v0.7.0. 2.2 is the first release
> that binds pre-provisioned machine credentials to an explicit authorization-server issuer
> (ADR-0024), fails closed on Protected Resource Metadata 429/5xx, and follows OAuth redirects only
> within origin. Earlier 2.x releases are no longer supported.

## Context

The package declares Python `>=3.13,<3.15` and MCP SDK `>=2.2,<3`. A normal lockfile-based
...
The minimum profile installs exactly MCP SDK 2.2.0. The latest profile resolves within
`mcp>=2.2,<3` at job execution time. The compatibility job starts from the locked project
